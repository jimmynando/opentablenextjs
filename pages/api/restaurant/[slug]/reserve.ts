...
import { findAvailableTables } from "../../../../services/restaurant/tables";
...
export default async function handler(
  ...
  if (req.method === "POST") {
    ...
    const restaurant = await prisma.restaurant.findUnique({
      select: { open_time: true, close_time: true, tables: true, id: true },
      where: { slug },
    });
    ...
    const searchTimeWithTables = searchTimesWithTables.find((t) => {
      return t.date.toISOString() === date.toISOString();
    });
